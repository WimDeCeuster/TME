...
{
    public class WhenPublishingSubModelAssets
    {
        [TheoryAttribute,
        InlineData("ext", "day"),
        ...
        public async Task ItShouldGroupTheAssetsCorrectly(string view, string mode)
        {
            ...
            var context = new ContextBuilder()
                ...
                .AddSubModelAsset(language, subModelID, objectId, asset2)
                .Build();
            ...
            // assert
            A.CallTo(() => assetService.PutSubModelAssetsByModeAndView(A<string>._, A<string>._, A<Guid>._, A<Guid>._, A<Guid>._, A<string>._, A<string>._, A<IEnumerable<Asset>>._))
            ...
            A.CallTo(() => assetService.PutSubModelAssetsByModeAndView(A<string>._, A<string>._, A<Guid>._, A<Guid>._, A<Guid>._, incorrectAssetType.Mode, incorrectAssetType.View, A<IEnumerable<Asset>>._))
                .MustHaveHappened(Repeated.Exactly.Once);

            A.CallTo(() => assetService.PutDefaultSubModelAssets(A<string>._, A<string>._, A<Guid>._, A<Guid>._, A<Guid>._, A<IEnumerable<Asset>>._))
                .MustHaveHappened(Repeated.Exactly.Once);
        } 
    }
}