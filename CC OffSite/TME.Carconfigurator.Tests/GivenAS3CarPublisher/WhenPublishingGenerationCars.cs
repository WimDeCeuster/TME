...
namespace TME.Carconfigurator.Tests.GivenAS3CarPublisher
{
    public class WhenPublishingGenerationCars : TestBase
    {
        ...
        protected override void Arrange()
        {
            ...
            A.CallTo(() => serialiser.Serialise(A<List<Car>>.That.IsSameSequenceAs(new List<Car> { car1 })))
                .Returns(_serialisedCar1);
            A.CallTo(() => serialiser.Serialise(A<List<Car>>.That.IsSameSequenceAs(new List<Car> { car1, car2 })))
                .Returns(_serialisedCar12);
            A.CallTo(() => serialiser.Serialise(A<List<Car>>.That.IsSameSequenceAs(new List<Car> { car3, car4 })))
                .Returns(_serialisedCar34);
            A.CallTo(() => serialiser.Serialise(A<List<Car>>.That.IsSameSequenceAs(new List<Car> { car4 })))
