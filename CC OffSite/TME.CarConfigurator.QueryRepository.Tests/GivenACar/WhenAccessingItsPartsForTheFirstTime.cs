...
{
    public class WhenAccessingItsPartsForTheFirstTime : TestBase
    {
        private ICar _car;
        private IReadOnlyList<ICarPart> _parts;
        ...
        {
            _parts.Should().HaveCount(2);
            _parts.Should().Contain(p => p.Code == _carPart1.Code);
            _parts.Should().Contain(p => p.Name == _carPart1.Name);
            _parts.Should().Contain(p => p.Code == _carPart2.Code);
            _parts.Should().Contain(p => p.Name == _carPart2.Name);
        }
    }
}