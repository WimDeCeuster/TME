...
{
    public class WhenRequestingDefaultAssetsForEquipmentOfDifferentCars : TestBase
    {
        ...
        protected override void Arrange()
        {
            ...
            _context = new ContextBuilder().WithBrand("brand").WithCountry("FR").WithLanguage("FR").Build();
            ...
            A.CallTo(() => assetService.GetCarEquipmentAssets(_publication.ID, _carId1, _context))
                .Returns(new Dictionary<Guid, List<Asset>> { { _equipmentId, _expectedAssets1 } });

            A.CallTo(() => assetService.GetCarEquipmentAssets(_publication.ID, _carId2, _context))
        ...
        {
            _assets1 = _assetFactory.GetCarEquipmentAssets(_publication, _carId1, _equipmentId, _context);
            _assets2 = _assetFactory.GetCarEquipmentAssets(_publication, _carId2, _equipmentId, _context);
        }

        [Fact]
        public void ThenItShouldReceiveTheCorrectListOfAssetsForCar1()
        {
            _assets1.Should().HaveCount(1);
        ...
        {
            _assets2.Should().HaveCount(2);

            _assets2.Should().Contain(a => a.ID == _expectedAssets2[0].ID);
            _assets2.Should().Contain(a => a.ID == _expectedAssets2[1].ID);
        }
    }
}