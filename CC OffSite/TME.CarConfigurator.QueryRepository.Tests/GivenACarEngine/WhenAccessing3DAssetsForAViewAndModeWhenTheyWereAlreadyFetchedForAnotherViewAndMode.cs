...
namespace TME.CarConfigurator.Query.Tests.GivenACarEngine
{
    public class WhenAccessing3DAssetsForAViewAndModeWhenTheyWereAlreadyFetchedForAnotherViewAndMode : TestBase
    {
        ...
        protected override void Arrange()
        {
            ...
            A.CallTo(() => _assetService.GetCarAssets(publication.ID, carId, repoEngine.ID, context, _view1, _mode1))
                .Returns(new List<Repository.Objects.Assets.Asset> { _asset1, _asset2 });
            A.CallTo(() => _assetService.GetCarAssets(publication.ID, carId, repoEngine.ID, context, _view2, _mode2))
            ...
            _fetchedAssets1 = _engine.VisibleIn.Single(v => v.Mode == _mode1 && v.View == _view1).Assets;
        }

        protected override void Act()
        {
            _fetchedAssets2 = _engine.VisibleIn.Single(v => v.Mode == _mode2 && v.View == _view2).Assets;
        }
