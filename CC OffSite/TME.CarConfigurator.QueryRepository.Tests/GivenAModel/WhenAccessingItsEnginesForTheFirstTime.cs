...
{
    public class WhenAccessingItsEnginesForTheFirstTime : TestBase
    {
        ...
        {
            _engines.Should().HaveCount(2);

            _engines.Should().Contain(engine => engine.ID == _engine1.ID);
            _engines.Should().Contain(engine => engine.ID == _engine2.ID);
        }
    }
}
