...
{
    public class WhenAccessingItsTransmissionsForTheFirstTime : TestBase
    {
        ...
        {
            _transmissions = _model.Transmissions;
        }

        [Fact]
        public void ThenItShouldFetchTheTransmissionsFromTheService()
        {
        ...
        {
            _transmissions.Should().HaveCount(2);

            _transmissions.Should().Contain(transmission => transmission.ID == _transmission1.ID);
            _transmissions.Should().Contain(transmission => transmission.ID == _transmission2.ID);
        }
    }
}
