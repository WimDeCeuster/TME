...
namespace TME.CarConfigurator.Query.Tests.GivenACarExteriorColour
{
    public class WhenAccessing3DAssetsForAModeAndViewWhenTheyWereAlreadyFetchedForAnotherModeAndView : TestBase
    {
        ...
        protected override void Arrange()
        {
            ...
            A.CallTo(() => _assetService.GetCarAssets(publication.ID, carId, repoCarExteriorColour.ID, context, VIEW1, MODE1)).Returns(new List<Asset> { _asset1, _asset2 });
            A.CallTo(() => _assetService.GetCarAssets(publication.ID, carId, repoCarExteriorColour.ID, context, VIEW2, MODE2)).Returns(new List<Asset> { _asset3 });
            ...
            _firstAssets = _exteriorColour.VisibleIn.Single(vi => vi.Mode == MODE1 && vi.View == VIEW1).Assets;
        }

        protected override void Act()
        {
            _secondAssets = _exteriorColour.VisibleIn.Single(vi => vi.Mode == MODE2 && vi.View == VIEW2).Assets;
        }
