...
namespace TME.CarConfigurator.Query.Tests.GivenAnUpholstery
{
    public class WhenAccessing3DAssetsForAViewAndModeWhenTheyWereAlreadyFetchedForAnotherViewAndMode : TestBase
    {
        ...
        protected override void Arrange()
        {
            ...
            A.CallTo(() => _assetService.GetAssets(publication.ID, repoUpholstery.ID, context, _view1, _mode1))
                .Returns(new List<Repository.Objects.Assets.Asset> { _asset1, _asset2 });
            A.CallTo(() => _assetService.GetAssets(publication.ID, repoUpholstery.ID, context, _view2, _mode2))
            ...
            _fetchedAssets1 = _upholstery.VisibleIn.Single(x=> x.Mode == _mode1 && x.View == _view1).Assets;
        }

        protected override void Act()
        {
            _fetchedAssets2 = _upholstery.VisibleIn.Single(x => x.Mode == _mode2 && x.View == _view2).Assets;
        }
