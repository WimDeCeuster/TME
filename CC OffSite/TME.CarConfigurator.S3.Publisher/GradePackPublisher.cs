...
namespace TME.CarConfigurator.S3.Publisher
{
    public class GradePackPublisher : IGradePackPublisher
    {
        ...
        private async Task<IEnumerable<Result>> Publish(string brand, string country, Guid publicationId, Guid timeFrameId, IReadOnlyDictionary<Guid, IList<GradePack>> gradePacks)
        {
            return await Task.WhenAll(gradePacks.Select(entry => _service.PutAsync(brand, country, publicationId, timeFrameId, entry.Key, entry.Value)));
        }
    }
}