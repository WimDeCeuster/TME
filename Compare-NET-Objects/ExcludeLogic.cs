...
namespace KellermanSoftware.CompareNetObjects
{
    ...
    internal static class ExcludeLogic 
    {
        ...
        {
            var config = parms.Config;

            //Only compare specific field names
            if (config.MembersToInclude.Count > 0 && !config.MembersToInclude.Contains(info.Name))
                return true;

            //If we should ignore the type member, skip it
            if (config.MembersToIgnore.Count > 0 &&
                (config.MembersToIgnore.Contains(info.Name) || config.MembersToIgnore.Contains(info.DeclaringType.FullName + "." + info.Name)))
                return true;

            //If we should ignore the member path, skip it
            var memberPath = parms.MemberPath + "." + info.Name;
            if (config.PathsToIgnore.Contains(memberPath))
                return true;
