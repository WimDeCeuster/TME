    ...
    public class ListComparer : BaseTypeComparer
    {
        ...
        public override void CompareType(CompareParms parms)
        {
            ...
            try
            {
                ...
                else if (parms.Config.DetectMissingAndMisordered)
                {
                    var missingAndMisorderedLogic = new MissingAndMisorderedLogic(RootComparer);
                    missingAndMisorderedLogic.CompareItems(parms);
                }
                else
                {
                    CompareItems(parms);
        ...
        private bool ListsHaveDifferentCounts(CompareParms parms)
        {
            ...
            {
                if (!parms.Config.DetectMissingAndMisordered)
                { 
        ...
        private void CompareItems(CompareParms parms)
        {
            ...
            while (enumerator1.MoveNext() && enumerator2.MoveNext())
            {
                ...
                var extra = parms.Config.CustomStringifier(enumerator1.Current) ?? String.Empty;

                if (extra.Length > 0)
                    extra = "(" + extra + ")";
                
                string currentBreadCrumb = AddBreadCrumb(parms.Config, parms.BreadCrumb, string.Empty, extra, count);
