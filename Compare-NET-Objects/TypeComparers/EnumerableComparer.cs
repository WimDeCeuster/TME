...
namespace KellermanSoftware.CompareNetObjects.TypeComparers
{
    ...
    public class EnumerableComparer :BaseTypeComparer
    {
        ...
        private object ConvertEnumerableToList(object source)
        {
            ...
            // look for IEnumerable<T> to reliable find the source element type (old implementation breaks for where selectors)
            var enumerableInterface = type.GetInterfaces().FirstOrDefault(i => i.GetGenericTypeDefinition() == typeof(IEnumerable<>));
            
            var genArgs = enumerableInterface == null ? type.GetGenericArguments() : enumerableInterface.GetGenericArguments();
