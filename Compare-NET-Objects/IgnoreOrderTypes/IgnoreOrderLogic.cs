...
namespace KellermanSoftware.CompareNetObjects.IgnoreOrderTypes
{
    ...
    public class IgnoreOrderLogic : BaseComparer
    {
            ...
            while (enumerator1.MoveNext())
            {
                ...
                if (!found)
                {
                    Difference difference = new Difference
                    {
                        ...
                        Object1Value = reverseCompare ? "(null)" : NiceString(enumerator1.Current, parms.Config.CustomStringifier),
                        Object2Value = reverseCompare ? NiceString(enumerator1.Current, parms.Config.CustomStringifier) : "(null)",
        ...
        private List<string> GetMatchingSpec(ComparisonResult result, Type type)
        {
            ...
            if (result.Config.CollectionMatchingSpec.Keys.Any(keyType => keyType.IsAssignableFrom(type)))
            {
                return result.Config.CollectionMatchingSpec.First(p => p.Key.IsAssignableFrom(type)).Value.ToList();
            }
